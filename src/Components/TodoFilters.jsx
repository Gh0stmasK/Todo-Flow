...
function TodoFilters({ currentFilter, stats, onFilterChange }) {
  ...
  return (
    <div className="flex items-center justify-center">
      <div className="inline-flex bg-gray-200 rounded-lg p-1 gap-2">
        {/*Inside the code I will add logic for filtering todos*/}
        {filters.map(({ key, label, icon: Icon, count }) => {
          return (
            <button
              className={`flex items-center gap-2 px-2 py-2 rounded-md text-sm font-medium transition all duration200 ${
                currentFilter === key
